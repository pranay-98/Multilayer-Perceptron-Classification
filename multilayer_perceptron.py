...
class MultilayerPerceptron:
    ...
    def _initialize(self, X, y):
        ...
        self._h_bias=1
        self._o_weights=np.random.rand(self.n_hidden,len(set(y)))
        self._o_bias=1
        np.random.seed(42)

        #raise NotImplementedError('This function must be implemented by the student.')

    def backward_propogation(self, train,hidden_layer,output_layer,target):
        loss_output=[]
        output_error=self._loss_function(target,output_layer)
        loss_output = ((-1)*(output_error) * self.hidden_activation(output_layer,derivative=True))

        

        for i in range(self.n_hidden):
            for j in range(len(target)):
                self._o_weights[i][j] -= (self.learning_rate * (loss_output[0][j] * hidden_layer[0][i]))
                
        
        loss_hidden = np.matmul(self._o_weights,loss_output.reshape(loss_output.shape[1],1))

        for i in range(len(target)):
            for j in range(self.n_hidden):
                
                self._h_weights[i][j] -= (self.learning_rate * (loss_hidden[j] * train[i]))
    ...
    def fit(self, X, y):
        ...
        for train,target in zip(X,self._y):
            prev_err=0
            for i in range(self.n_iterations):

                h_values=np.dot(train.reshape(1,X.shape[1]),self._h_weights)
                h_values+=self._h_bias
                norm_data=self.hidden_activation(h_values)

                output_vals=np.dot(norm_data,self._o_weights)+self._o_bias


                predict_vals=self._output_activation(output_vals)

                self.backward_propogation(train,norm_data,predict_vals,target)

               
                for i in range(len(predict_vals)):
                    erro = (predict_vals[i] - target[i])**2
                    square_error=sum(erro)

                curr_err= square_error


                if np.absolute(curr_err-prev_err)<0.1:
                    break
    ...
    def predict(self, X):
        ...
        for test in X:
            h_values=(np.dot(test.reshape(1,X.shape[1]),self._h_weights))+self._h_bias

            layer_out=self.hidden_activation(h_values)

            output_vals=np.dot(layer_out,self._o_weights)+self._o_bias

            predict_vals=self._output_activation(output_vals)

            predicted_classes.append(np.argmax(predict_vals))
